...
(async()=>{
 ...
 async function worker(){
  ...
  try{
   ...
   for(const [side,c] of [['picker:A:0',cards[0]],['picker:B:0',cards[1]]]){const sideLabel=side==='picker:A:0'?'left':'right';await p.locator(`[data-open-picker="${side}"]`).click();await p.locator('[data-filter="search"]').fill(c.name);await p.locator(`[data-browser-detail="${c.id}"]`).click();if(index<6)await p.screenshot({path:path.join(OUT,`review-${index+1}-${sideLabel}-detail.png`)});await p.locator('[data-detail-select]').click();}
   ...
   while(Date.now()-start<600000){const state=await p.evaluate(()=>({round:window.__battle?.round,frame:document.querySelector('#view-battle').dataset.frameIndex,text:document.querySelector('.current-event')?.textContent,ended:window.__battle?.outcome().ended,last:window.__battle?.presentationIndex - 1,hp:[...document.querySelectorAll('.entity-card .meter-group')].map(el=>el.innerText)}));if(observed.at(-1)?.frame!==state.frame)observed.push(state);
