(function(root){
  ...
  const PARAMETER_LIST=[
    ...
    p('RES_PEN','法术穿透','实体基础','number','%',0,'0–95','按比例忽略目标的 RES。','Stat-only V7 战斗中作为魔法伤害穿透，与 PEN（物理）互补。','只影响魔法伤害部分，不影响 DEF 相关物理减免。','RPG spell penetration'),
    ...
    p('HP_REGEN','每回合回复','实体基础','number','%',0,'0–12','每回合回复 MAX_HP 的百分比。','回合开始时按 MAX_HP×HP_REGEN/100×(HEAL_POWER/100)×(HEAL_TAKEN/100) 回复。','与 LIFESTEAL 同受 HEAL_POWER/HEAL_TAKEN 调节；上限为 MAX_HP。','RPG health regen per round'),
    p('POTENCY','效能','实体基础','number','point',100,'0–500','放大周期、触发与引爆类非直接数值效果。','以 100 为中性值，经有界递减曲线进入非直接伤害。','不改变普通直接伤害，避免 ATK 同时统治所有输出渠道。','RPG spell/status potency'),
    p('CONTROL_POWER','控制强度','实体基础','number','point',100,'0–500','提高敌对非 DoT 状态的成功率和有限持续时间。','与目标 TENACITY 在 logit 概率空间对抗。','双方同为 100 时严格保留基础 chance；持续时间限制在 0.60–1.50 倍。','RPG control power'),
    p('TENACITY','韧性','实体基础','number','point',100,'0–500','抵抗敌对控制与弱化状态。','作为 CONTROL_POWER 的对抗轴降低最终 chance 和有限 duration。','只影响敌对非 DoT 状态，不抵消伤害或友方增益。','RPG tenacity/control resistance'),
    p('RECOVERY','恢复速度','实体基础','number','point',100,'0–500','改变技能冷却准备速度。','每回合累积 cooldownProgress 并跨整数减少冷却。','100 严格等于每回合 1 点；不会增加每回合行动机会。','RPG cooldown recovery'),
    p('BARRIER_POWER','屏障强度','实体基础','number','point',100,'0–500','放大 Shield、Ward 与 Barrier 数值。','以 100 为中性值，经有界递减曲线放大屏障类效果。','与 MAX_HP 分离，减少生命同时控制基础耐久和所有屏障的双重收益。','RPG barrier power'),
    p('TOUGHNESS','坚韧','实体基础','number','%',0,'0–30','Stat-only V7 的固定伤害减免。','对最终伤害乘以 (1 - TOUGHNESS/100)，有界 0–30%。','与 DEF/RES 乘算；高投入高收益但受上界限制。','RPG flat damage reduction'),
    p('CRIT_RES','暴击抵抗','实体基础','number','%',0,'0–50','Stat-only V7 中降低受到的暴击加成。','敌方暴击时的额外倍率乘以 (1 - CRIT_RES/100)。','只削减暴击部分，不影响普通命中伤害。','RPG critical resistance'),
