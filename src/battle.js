...
(function (global) {
  ...
  function act(unit, foe, rng, add, label) {
    ...
    add(`　${label(unit)} 对 ${label(foe)} 造成 ${fmt(dealt)} 伤害${isCrit ? '（暴击！）' : ''}`, isCrit ? 'crit' : 'hit');
    // 吸血
    if (unit.lifesteal > 0 && dealt > 0 && unit.hp < unit.maxHp) {
      ...
      unit.hp += heal;
      add(`　${label(unit)} 吸取 ${fmt(heal)} HP`, 'heal');
    }
    if (foe.hp <= 0) {
      foe.alive = false;
      add(`💀 ${label(foe)} 倒下了！`, 'death');
    }
  }
  ...
  function simulate(cardA, levelA, cardB, levelB, seed) {
    ...
    A.alive = true;
    B.alive = true;
    const label = u => `${u === A ? '蓝方' : '红方'}·${u.name}`;
    const events = [];
    let round = 0;
    ...
    add(`⚔ 战斗开始 — ${label(A)} Lv.${A.level} [${A.rarityName}] VS ${label(B)} Lv.${B.level} [${B.rarityName}] · MatchSeed ${seed}`, 'sys');
    ...
    while (A.alive && B.alive && round < MAX_ROUNDS) {
      ...
      act(goFirst, goSecond, rng, add, label);
      if (A.alive && B.alive) act(goSecond, goFirst, rng, add, label);
    }
    ...
    add(
      winner === 0 ? `🏆 ${label(A)}（Lv.${A.level} ${A.rarityName}）获胜` :
      winner === 1 ? `🏆 ${label(B)}（Lv.${B.level} ${B.rarityName}）获胜` :
      '⚖ 平局（回合上限，按剩余生命比例判定）',
      winner === -1 ? 'draw' : 'sys'
    );
