(function(root){
  ...
  function cardInfo(card){
    const signature=N.cardPresentationSignature(card);
  ...
  class CardBrowser{
    ...
    restoreControls(){for(const [k,v] of Object.entries(this.filters)){const input=this.container.querySelector(`[data-filter="${k}"]`);if(input)input.value=v;}const bucket=this.container.querySelector('[data-level-bucket]');if(bucket)bucket.value=`${this.filters.min||''}-${this.filters.max||''}`;}
    results(){
    ...
    detail(card){
      ...
      const statRows=Object.entries(card.stats||{}).map(([k,v])=>`<div class="detail-row"><button class="detail-stat" data-knowledge="${esc(k)}"><span>${esc(N.knowledgeLookup?.(k)?.entry?.nameZh||k)} <small>${esc(k)}</small></span><b>${esc(v)}</b><i aria-label="查看说明">ⓘ</i></button></div>`).join('');
