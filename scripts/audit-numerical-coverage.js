...
// large v4 sample; any field/effect/condition/event used by real content without
// a knowledge entry FAILS. Also verifies the generated reference is current
// (drift detection) and counts documented coverage.
...
function audit(sample=10000){
  const cards=require(path.join(ROOT,'content/presets-v4.json')).cards;
  const gaps=[];
  for(const c of cards)gaps.push(...N.knowledgeCoverageGaps(c).map(g=>'preset '+c.displayName+': '+g));
  // diversity sample
  for(let i=0;i<sample;i++){
    const c=N.generateCardV4({seed:'nk-audit-'+i,rarity:N.RARITY_V2_ORDER[i%12],level:10+(i*37)%91});
    gaps.push(...N.knowledgeCoverageGaps(c).map(g=>'gen '+i+': '+g));
  }
  ...
  return{
    presets:cards.length,v4Sample:sample,
...
if(require.main===module){
  ...
  console.log(JSON.stringify({presets:r.presets,v4Sample:r.v4Sample,undocumentedActiveFields:r.undocumentedActiveFields,params:r.params,effects:r.effects,conditions:r.conditions,targets:r.targets,events:r.events,formulaSymbols:r.formulaSymbols,formulaFunctions:r.formulaFunctions,damageTypes:r.damageTypes,gapSample:r.gaps.slice(0,12)}));
