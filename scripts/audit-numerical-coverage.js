...
// large v4+v5 sample; any field/effect/condition/event used by real content
...
for(const f of ['kernel','components','rules','content','status-runtime','formula','validator','effects','engine','ai','power','power-v5','battlepower-v3','gen-stats','gen-skills','generator','gen-names','name-generator-v2','gen-v2','gen-v3','gen-v4','gen-v5','behavior','battlepower-v2','numerical-knowledge'])require(path.join(ROOT,'src',f+'.js'));
const N=global.NCB;
function audit(sample=10000){
  const cards=require(path.join(ROOT,'content/presets-v5.json')).cards;
  const gaps=[];
  for(const c of cards)gaps.push(...N.knowledgeCoverageGaps(c).map(g=>'preset '+c.displayName+': '+g));
  // diversity sample (v4 legacy + v5 default)
  for(let i=0;i<sample;i++){
    const c=N.generateCardByVersion({seed:'nk-audit-v5-'+i,rarity:N.RARITY_V2_ORDER[i%12],level:10+(i*37)%91});
  ...
  return{
    presets:cards.length,v4Sample:sample,v5Sample:sample,
...
if(require.main===module){
  ...
  console.log(JSON.stringify({presets:r.presets,v4Sample:r.v4Sample,v5Sample:r.v5Sample,undocumentedActiveFields:r.undocumentedActiveFields,params:r.params,effects:r.effects,conditions:r.conditions,targets:r.targets,events:r.events,formulaSymbols:r.formulaSymbols,formulaFunctions:r.formulaFunctions,damageTypes:r.damageTypes,gapSample:r.gaps.slice(0,12)}));
