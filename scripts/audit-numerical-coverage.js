...
// large v4+v5+v6 sample plus all V7 skeleton families; any active field/effect/condition/event
...
function audit(sample=10000){
  ...
  const v7Sample=Math.min(sample,512);
  for(let i=0;i<v7Sample;i++){
    const c7=N.generateCardV7({seed:'nk-audit-v7-'+i,rarity:N.RARITY_V2_ORDER[i%12],level:10+(i*37)%91});
    gaps.push(...N.knowledgeCoverageGaps(c7).map(g=>'gen7 '+i+': '+g));
  }
  ...
  return{
    presets:cards.length,v4Sample:sample,v5Sample:sample,v6Sample:sample,v7Sample,
...
if(require.main===module){
  ...
  console.log(JSON.stringify({presets:r.presets,v4Sample:r.v4Sample,v5Sample:r.v5Sample,v6Sample:r.v6Sample,v7Sample:r.v7Sample,undocumentedActiveFields:r.undocumentedActiveFields,params:r.params,effects:r.effects,conditions:r.conditions,targets:r.targets,events:r.events,formulaSymbols:r.formulaSymbols,formulaFunctions:r.formulaFunctions,damageTypes:r.damageTypes,gapSample:r.gaps.slice(0,12)}));
